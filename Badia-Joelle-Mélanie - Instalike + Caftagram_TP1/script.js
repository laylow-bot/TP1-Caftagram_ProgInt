...
function displayPostsElements(i, resPosts, resUsers, descriptionDetails) {
  ...
  const divUserPhoto = document.createElement('div'); // Added div for user photo
  //const imageUser = document.createElement('img'); // Added image element for user photo
  ...
  divContainer.setAttribute('class', 'container aligns-items-center mb-3');
  ...
  buttonLike.setAttribute('class', 'btn btn-outline-success');
  ...
  //divUserPhoto.setAttribute('class', 'user-photo');
  //imageUser.setAttribute('src', 'path_to_user_photo.jpg'); // Replace 'path_to_user_photo.jpg' with the path to your user photo
  //imageUser.setAttribute('alt', 'User Photo');
  //divUserPhoto.appendChild(imageUser);
  ...
  divCart.appendChild(linkUsername);
  ...
  // Append user photo to the card body
  divCart.appendChild(divUserPhoto);
  ...
  containerPosts.appendChild(divContainer);
}
    ...
    .then(resPosts => {
      ...
      fetch('https://insta-api-api.0vxq7h.easypanel.host/users')
        ...
        .then(resUsers => {
          ...
          newDivCard.setAttribute("class", "container aligns-items-center, justify-content-center mb-3")
          ...
          if (cmpt == 0) {
            const newDescription = document.createElement('p')
            newDescription.textContent = "Aucune publication disponible. Refaire la recherche svp"
            newDivCard.appendChild(newDescription)
            containerPosts.appendChild(newDivCard)
          }
        })
...
function createComments(postIdComments) {
  ...
  if (content.length == 0) {
    popupContent.textContent = "Vous devez inscrire un commentaire";
...
function createLikes(postIdLike) {
  ...
  fetch("https://insta-api-api.0vxq7h.easypanel.host/likes", {
    ...
    .then(data => {
      ...
      popupContent.textContent = "votre like sur cette publication a été enregistré";
      popup.style.display = "block";
